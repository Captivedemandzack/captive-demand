            ...
            <div className="flex flex-col md:flex-row gap-8 md:gap-0">
                <div className="flex-1 md:pr-8 flex flex-col">
                    ...
                    <div className="mt-auto">
                        <div className="flex items-center justify-between mb-4">
                            <h4 className="text-base font-medium">March 2026</h4>
                        ...
                        <div className="grid grid-cols-7 mb-2">
                            {days.map((day) => (
                                <div key={day} className="text-center text-[10px] text-white/30 font-medium py-2">{day}</div>
                            ))}
                        </div>
                        <div className="grid grid-cols-7 gap-y-1 gap-x-1">
                            {calendarDays.map((item, index) => (
                                <div key={index} className="aspect-square flex items-center justify-center">
                                    {item.day && (
                                        <button
                                            className={`w-8 h-8 md:w-9 md:h-9 rounded-full text-sm font-medium transition-all flex items-center justify-center ${
                                                item.selected ? 'bg-white text-[#1a1512]'
                                                    : item.available ? 'bg-[#2a2522] text-white hover:bg-[#3a3532]'
                ...
                <div className="flex-1 md:border-l border-white/10 md:pl-8 flex flex-col">
                    <div className="flex items-center justify-between mb-6 pt-2">
                        <span className="text-white font-medium">Thu 3</span>
                    ...
                    <div className="flex-1 space-y-3">
                        {timeSlots.map((time, i) => (
                            <button
                                key={i}
                                className="w-full text-center py-3.5 rounded-xl border border-white/10 text-white font-medium bg-transparent hover:border-white/40 hover:bg-white/5 transition-all duration-200 active:scale-[0.98] text-sm"
                            >
                                {time}
                            </button>
                        ))}
